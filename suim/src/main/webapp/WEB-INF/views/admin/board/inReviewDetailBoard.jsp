...
<title>자유게시판</title>
	...
	<div class="content">
        ...
        <div class="innerOuter">
			...
			<button id="backButton" class="btn btn-secondary" style="float:right; background-color: rgb(250, 107, 111);">목록으로</button>
			<script>
			...
			success : function(result) {
				 ...
				 let resultStr = "";
		            ...
		            for (let i = 0; i < result.length; i++) {
		                resultStr += "<tr data-id=" + result[i].ireNo + ">" +
		                ...
		                if (true) {
		                    resultStr += "<td style='width: 70px;'><button class='btn btn-danger deleteButton' data-id='" + result[i].memberId + "' style='float:right; background-color: rgb(250, 107, 111); font-size: 11px; padding: 4px 8px;'>" +
		                        ...
		                        "</button></td>";
		                    resultStr += "<td style='width: 70px;'><button class='btn btn-secondary updateButton' data-id='" + result[i].memberId + "' data-content='" + result[i].ireContent + "' style='float:right; background-color: #7464a1; font-size: 11px; padding: 4px 8px;'>" +
	...
	$(document).off("click", ".updateButton").on("click", ".updateButton", function() {
	    ...
	    let ireNo = $(this).closest("tr").data("id");
	    ...
	    $("#updateForm").off("submit").on("submit", function(e) {
	        ...
	        // 알림 대화상자 표시
	            ...
	            $.ajax({
	                url: "/rupdate.in",
	                data: { ire: ireNo, content: updateContent },
	                type: "post",
	                success: function(response) {
	                        // 댓글 내용 업데이트
						    ...
						    let targetRow = $("tr[data-id='" + ireNo + "']");
