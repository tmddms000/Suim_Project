...
<style>
	#findBoardList {text-align:center;}
	#findBoardList>tbody>tr:hover {
		cursor:pointer;
        ...
        <div class="content">
                ...
                <div class="row g-4">
                     ...
                     <div class="col-12">
                        <div class="bg-light rounded h-100 p-4" width="">
							...
							<form class="d-md-flex ms-4" action="/adminFindKeySearch.bo" method="get">
								<div class="select">
							...
							<form id="enrollForm" method="get" action="/adminFindDeSearch.bo" enctype="multipart/form-data">     
								<div class="search-filter">
								    ...
								    <label for="category">조건:</label>
								    <select id="category" name="category" style="border-radius: 8px;">
                            ...
                            <div class="tab-content pt-3" id="nav-tabContent">
                                <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab">
									<div class="table-responsive">
										<form action="/adminFindDelete.bo" method="post" >
		                                <table class="table" id="findBoardList">
		                                    <thead>
		                                        <tr>
		                                    ...
		                                    <c:choose>
			                                    <c:when test="${ not empty list }">
			                                    	<c:forEach var="f" items="${ list }">
				                    					<tr>
					                                      	<td scope="row"><input class="form-check-input" type="checkbox" name="checkboxName" value="${f.findNo}"></td>
					                                        <td class="fno">${ f.findNo }</td>
					                                        <td>${ f.findTitle }</td>
					                                        <td>${ f.memberId }</td>
					                                        <td>${ f.category }</td>
					                                        ...
					                                        <td>${ f.findDate }</td>
					                                        <td>${ f.findView }</td>
																<c:if test="${f.status eq 'Y'}">
																    <td>등록</td>
																</c:if>
																<c:if test="${f.status eq 'N'}">
																    <td>삭제</td>
																</c:if>			
															<td>
																<a href="/adminFindUpdateForm.bo?fno=${f.findNo}">
				...
				<nav id="pagingArea" style="margin-top: 30px; margin-bottom: 30px;">
					<ul class="pagination justify-content-center">
						<li class="page-item ${pi.currentPage == 1 ? 'disabled' : ''}">
							<a class="page-link" href="<c:url value='adminFindList.bo?currentPage=${pi.currentPage - 1}'/>">&lt;</a>
						</li>
						<c:forEach var="p" begin="${pi.startPage}" end="${pi.endPage}" step="1">
							<li class="page-item ${pi.currentPage == p ? 'active' : ''}">
								<a class="page-link" href="<c:url value='adminFindList.bo?currentPage=${p}'/>">${p}</a>
							</li>
						</c:forEach>
			...
			<c:if test="${not empty param.del and empty param.condition}">
				<nav id="pagingArea" style="margin-top: 30px; margin-bottom: 30px;">
					<ul class="pagination justify-content-center">
						<li class="page-item ${pi.currentPage == 1 ? 'disabled' : ''}">
							<a class="page-link" href="<c:url value='adminFindDeSearch.bo?currentPage=${pi.currentPage - 1}&gender=${gender}&category=${category}&del=${del}'/>">&lt;</a>
						</li>
						<c:forEach var="p" begin="${pi.startPage}" end="${pi.endPage}" step="1">
							<li class="page-item ${pi.currentPage == p ? 'active' : ''}">
								<a class="page-link" href="<c:url value='adminFindDeSearch.bo?currentPage=${p}&gender=${gender}&category=${category}&del=${del}'/>">${p}</a>
							</li>
						</c:forEach>
						<li class="page-item ${pi.currentPage == pi.maxPage || pi.listCount == 0 ? 'disabled' : ''}">
							<a class="page-link" href="<c:url value='adminFindDeSearch.bo?currentPage=${pi.currentPage + 1}&gender=${gender}&category=${category}&del=${del}'/>">&gt;</a>
			...
			<c:if test="${empty param.del and not empty param.condition}">
				<nav id="pagingArea" style="margin-top: 30px; margin-bottom: 30px;">
					<ul class="pagination justify-content-center">
						<li class="page-item ${pi.currentPage == 1 ? 'disabled' : ''}">
							<a class="page-link" href="<c:url value='adminFindKeySearch.bo?currentPage=${pi.currentPage - 1}&condition=${condition}&keyword=${keyword}'/>">&lt;</a>
						</li>
						<c:forEach var="p" begin="${pi.startPage}" end="${pi.endPage}" step="1">
							<li class="page-item ${pi.currentPage == p ? 'active' : ''}">
								<a class="page-link" href="<c:url value='adminFindKeySearch.bo?currentPage=${p}&condition=${condition}&keyword=${keyword}'/>">${p}</a>
							</li>
						</c:forEach>
						<li class="page-item ${pi.currentPage == pi.maxPage || pi.listCount == 0 ? 'disabled' : ''}">
							<a class="page-link" href="<c:url value='adminFindKeySearch.bo?currentPage=${pi.currentPage + 1}&condition=${condition}&keyword=${keyword}'/>">&gt;</a>
