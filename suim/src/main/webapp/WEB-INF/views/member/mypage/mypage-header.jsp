        ...
        <div class="profile">
          <div class="profile-header">
            ...
            <div class="profile-header-content">
              <div class="profile-header-img">
                <c:choose>
                  ...
                  <c:otherwise>
                    <img src="<c:out value='${loginUser.changeName}'/>" alt="사진" style="width: 100%; height: 100%">
                  </c:otherwise>
                </c:choose>
              </div>
              <div class="profile-header-info">
                <c:choose>
                  <c:when test="${not empty loginUser}">
                    <h4 class="m-t-10 m-b-5"><c:out value='${loginUser.nickName}'/></h4>
                    <c:choose>
                      <c:when test="${not empty loginUser.memberComment}">
                        <p class="m-b-10"><c:out value='${loginUser.memberComment}'/></p>
                      </c:when>
                      <c:otherwise>
                        <p class="m-b-10">회원 메시지가 없습니다</p>
                      </c:otherwise>
                    </c:choose>
                    <button type="button" class="editModalBtn" data-bs-toggle="modal" data-bs-target="#myModal" style="color: white; height: 40px; background: transparent;"><i class="fa-solid fa-pen"></i> 수정하기</button>
                  </c:when>
                  <c:otherwise>
            ...
            <ul class="profile-header-tab nav nav-tabs">
              ...
              <li class="nav-item">
                <a href="/mypage/wish" class="nav-link ${fn:startsWith(pageUrl, 'wish') ? 'actives' : ''}"><i class="fa-solid fa-heart-circle-plus"></i> 찜목록</a>
              </li>
              <li class="nav-item">
                <a href="/mypage/house" class="nav-link ${fn:startsWith(pageUrl, 'house') ? 'actives' : ''}"><i class="fa-solid fa-house"></i> 셰어하우스</a>
              </li>
              <li class="nav-item">
  ...
  <div class="modal-dialog modal-dialog-centered" style="max-width: 400px !important;">
    <div class="modal-content">
      ...
      <div class="modal-body" style="text-align: left">
        <form id="updateForm" method="post" action="/mypage/updateProfile" enctype="multipart/form-data">
          <div id="fileupload_profile_img2" class="fileupload_profile_img">
            ...
            <span class="over"><i class="fa-solid fa-camera" style="color: #f2f2f2; margin-top: 7px;"></i></span>
            <div id="drop_profile_img2" class="drop_profile_img">
              <input title="프로필 업로드" type="file" name="file" style="width: 100%; height: 100%; cursor: pointer;" accept="image/jpeg, image/jpg, image/png, image/gif, image/svg+xml">
            </div>
          </div>
          <input class="form-control" type="text" name="nickName" placeholder="닉네임" value="${ loginUser.nickName }" required="required" style="width: 150px; margin: 0 auto; text-align: center; margin-bottom: 25px;">
          <input class="form-control" type="text" name="memberComment" placeholder="프로필 메시지" value="${ loginUser.memberComment }" style="width: 300px; margin: 0 auto; text-align: center;">
          <div class="input-border" style="height: 760px; width: 349px; margin: 20px auto;">
            <div class="input_block form-group">
              <div class="input_form _item _uid">
                <input title="아이디" type="text" name="memberId" readonly="readonly" value="${ loginUser.memberId }" placeholder="이메일" required="required">
                <i aria-hidden="true" class="zmdi zmdi-check"></i>
                <div class="alert-block _msg"></div>
              </div>
              <div class="input_block form-group">
                <div class="input_form _item _uid">
                  <input title="이메일" type="email" name="email" readonly="readonly" value="${ loginUser.email }" placeholder="이메일" required="required">
                  <i aria-hidden="true" class="zmdi zmdi-check"></i>
                  <div class="alert-block _msg"></div>
				</div>
                <div class="input_form brt _item _passwd_org">
                  <input title="기본 비밀번호 입력" type="password" name="memberPwd" autocomplete="off" placeholder="정보를 변경하려면 비밀번호를 입력하세요" required>
                  <i aria-hidden="true" class="zmdi zmdi-check"></i>
                  <div class="alert-block _msg"></div>
                </div>
                <div class="input_form brt _item _passwd">
                  <input title="비밀번호" type="password" name="changePwd1" autocomplete="off" placeholder="비밀번호를 변경하는 경우 입력하세요">
                  <i aria-hidden="true" class="zmdi zmdi-check"></i>
                  <div class="alert-block first-letter-uppercase _msg"></div>
                </div>
                <div class="input_form brt _item _passwd_confirm">
                  <input title="비밀번호 확인" type="password" name="changePwd2" autocomplete="off" placeholder="비밀번호 변경 확인">
                  <i aria-hidden="true" class="zmdi zmdi-check"></i>
                  <div class="alert-block _msg"></div>
                </div>
              </div>
              <div class="input_block form-group">
                ...
                <div class="input_form _item _name">
                  <input title="이름을 입력하세요" type="text" id="join_name" name="memberName" placeholder="이름을 입력하세요" value="${ loginUser.memberName }" required="required">
                </div>
              </div>
              <div class="input_block form-group">
                <label class="mini-tit" for="join_name">성별<span style="color: red">*</span></label>
                <select title="성별" class="form-select" id="birth_y" name="gender">
                  <option value="M" <c:if test="${loginUser.gender eq 'M'}">selected</c:if>>남자</option>
                  <option value="F" <c:if test="${loginUser.gender eq 'F'}">selected</c:if>>여자</option>
                </select>
              </div>
              <div class="input_block form-group">
                <label class="mini-tit" for="join_name">연락처</label>
                <input class="input_form" type="tel" id="join_callnum" name="phone" placeholder="연락처" value="${ loginUser.phone }" required="required">
                <p class="good-text">연락처는 가맹 인증 시 사용됩니다.</p>
              </div>
              <c:set var="birthYear" value="${loginUser.birth.substring(0, 4)}" />
              <c:set var="birthMonth" value="${loginUser.birth.substring(4, 6)}" />
              <c:set var="birthDay" value="${loginUser.birth.substring(6, 8)}" />
              <div class="input_block form-group">
                <label class="mini-tit" for="join_name">생년월일<span style="color: red">*</span></label>
                <div class="info" id="info__birth" align="left">
                  ...
                  <input type="hidden" name="birth" id="birth-date-input" value="${birthYear}${birthMonth}${birthDay}">
                </div>
              </div>
              ...
              <div class="text-center">
                <c:if test="${not empty loginUser.naverLogin && loginUser.naverLogin != null}">
                  ...
                  <span style="vertical-align: -5px;"><img src="/resources/img/member/naverBtn.png" style="width: 20px; height: 20px;"></span> 연결됨
                  <input type="hidden" name="naverLogin" value="${loginUser.naverLogin}" />
                </c:if>
                <c:if test="${not empty loginUser.googleLogin && loginUser.googleLogin != null}">
                  <p class="good-text">소셜로그인 상태</p>
                  <span style="vertical-align: -5px;"><img src="/resources/img/common/google.jpg" style="width: 20px; height: 20px;"></span> 연결됨
                  <input type="hidden" name="googleLogin" value="${loginUser.googleLogin}" />
                </c:if>
                <c:if test="${empty loginUser.naverLogin && loginUser.naverLogin == null && empty loginUser.googleLogin && loginUser.googleLogin == null}">
                  <p class="good-text">소셜로그인 상태</p>
          ...
          <div class="modal-footer justify-content-center">
            <button type="submit" class="btn btn-primary" style="width: 250px;">확인</button>
          </div>
        </form>
      </div>
    </div>
