							...
							<c:when test="${not empty list}">
							  <c:forEach var="b" items="${list}">
							    <tr data-id="${type eq 'board' ? b.boardNo : (type eq 'find' ? b.findNo : b.inrNo)}">
							      <td class="select-cell"><input type="checkbox" data-id="${type eq 'board' ? b.boardNo : (type eq 'find' ? b.findNo : b.inrNo)}"></td>
							      <td class="boardno-cell">${type eq 'board' ? b.boardNo : (type eq 'find' ? b.findNo : b.inrNo)}</td>
							      <td class="title-cell"><a href="/detail.bo?bno=${type eq 'board' ? b.boardNo : (type eq 'find' ? b.findNo : b.inrNo)}">${type eq 'board' ? b.boardTitle : (type eq 'find' ? b.findTitle : b.inrTitle)}</a> <a href="/detail.bo?bno=${type eq 'board' ? b.boardNo : (type eq 'find' ? b.findNo : b.inrNo)}#replyArea"><c:if test="${type eq 'board' ? b.replyCount > 0 : (type eq 'find' ? b.freplyCount > 0 : b.replyCount > 0)}"><span class="reply-cell">[${type eq 'board' ? b.replyCount : (type eq 'find' ? b.freplyCount : b.replyCount)}]</span></c:if></a></td>
